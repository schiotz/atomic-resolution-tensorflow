...
graph_path = os.path.join(graph_dir, 'cluster-110-single-class-{:d}.h5')
...
def makeimage(entry):
    ...
    return image[:,:328,:328,:],label[:,:328,:328,:]
...
class MakeImages:
    ...
    def precompute(self):
        ...
        with Pool() as pool:
            self.precomputed = deque(pool.map(makeimage, entries))
    ...
    def get_all_examples(self):
        ...
        labels = []
        with Pool() as pool:
            # Is the list() necessary to force computation to occur immediately?
            for img, lbl in list(pool.map(makeimage, entries)):
                images.append(img)
                labels.append(lbl)
...
imagestream_train = MakeImages(data_train)
n_train = data_train.num_examples
data_valid = load(validation_dir)
imagestream_valid = MakeImages(data_valid)
