...
validation_dir = "data/graphene-random-test/"
...
def load_CNN(graph_path, num_gpus, size):
...
def local_eval(prediction, label):
    return evaluate_result(prediction, label, sampling, threshold=0.5, accept_distance=1.0)
...
with open(result, "wt") as outfile:
    ...
    for step, gr in cnnfiles:
        ...
        i += 1
        x, model = load_CNN(gr, num_gpus, image_size)
        ...
        for (n, imagestream) in ((n_train, imagestream_train), (n_valid, imagestream_valid)):
            ...
            with Pool(maxcpu) as pool:
                result = pool.starmap(local_eval, zip(predictions, labels))
