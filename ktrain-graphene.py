...
def makeimage(entry, size, imgnum, rndnums):
    ...
    sampling = rnd(.11,.13)
    normalizerange = 12.0 / 0.12
    spotsize = 0.3  # Size of spots in ground truth - normally 0.4
    Cs = rnd(-20,20) * 1e4
    #Cs = rnd(-30,0) * 1e4
    defocus = rnd(-100,100)
    focal_spread = rnd(20,40)
    
    aberrations={'a22' : rnd(0, 30), 
                'phi22' : rnd(0, 2 * np.pi),
    ...
    c1=1.0   # 1.0 in paper
    c2=rnd(0,.1)  # c1 in paper
    c3=rnd(.4,.6)  # c2 in paper
    c4=rnd(2.0, 3.0)  # c3 in paper
